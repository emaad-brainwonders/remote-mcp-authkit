...
const HARDCODED_GOOGLE_ACCESS_TOKEN = "ya29.a0AS3H6NyvGbk5Y494bZlPzLPql8tBUSgWOj06eb1s-5XJjyImEC6DENg1o_EkWoDpUbi2ZVKVEN8o5YuBq6n-FaTo0PbAsSqCrZEOdINhEj-Fde_FVsfiv4_vYPSFOvf-QEcJ8p2V9htnC6EmI04eD4zDaLRuHSAeOXvJfKvERwaCgYKAYISARQSFQHGX2Mi9tT270k2eXtzkC6J18yUXQ0177";
...
export function registerAppointmentTools(server: McpServer) {
	...
	server.tool(
	"cancelAppointment",
	"Cancel an appointment by matching attendee email and optionally summary/date.",
	{
		email: z.string().email().describe("Attendee email address"),
		summary: z.string().optional().describe("(Optional) Event summary to match"),
		date: z.string().optional().describe("(Optional) Event date in YYYY-MM-DD format or relative form")
	},
	async ({ email, summary, date }) => {
		try {
			const parsedDate = date ? parseRelativeDate(date) : undefined;
			const timeMin = parsedDate ? `${parsedDate}T00:00:00+05:30` : undefined;
			const timeMax = parsedDate ? `${parsedDate}T23:59:59+05:30` : undefined;

			let query = `https://www.googleapis.com/calendar/v3/calendars/primary/events?singleEvents=true&orderBy=startTime`;
			if (timeMin && timeMax) {
				query += `&timeMin=${encodeURIComponent(timeMin)}&timeMax=${encodeURIComponent(timeMax)}`;
			}

			const events = (await makeCalendarApiRequest(query)).items || [];

			const matching = events.find(event => {
				const hasAttendee = (event.attendees || []).some((a: any) => a.email === email);
				const matchesSummary = summary ? event.summary === summary : true;
				return hasAttendee && matchesSummary;
			});

			if (!matching) {
				return { content: [{ type: "text", text: `❌ No appointment found with attendee ${email}${summary ? ` and title "${summary}"` : ''}${parsedDate ? ` on ${parsedDate}` : ''}.` }] };
			}

			await makeCalendarApiRequest(
				`https://www.googleapis.com/calendar/v3/calendars/primary/events/${matching.id}`,
				{ method: "DELETE" }
			);

			return { content: [{ type: "text", text: `✅ Appointment with ${email} has been successfully cancelled.` }] };
		} catch (error) {
			return { content: [{ type: "text", text: `❌ Failed to cancel appointment. ${error instanceof Error ? error.message : 'Unknown error.'}` }] };
		}
	}
);
...
server.tool(
	...
	"Reschedule an appointment based on attendee email.",
	{
		email: z.string().email().describe("Attendee email address to identify the event"),
		newDate: z.string().describe("New date in YYYY-MM-DD format or relative form"),
		newStartTime: z.string().regex(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/).describe("New start time in HH:MM format"),
		newEndTime: z.string().regex(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/).describe("New end time in HH:MM format")
	},
	async ({ email, newDate, newStartTime, newEndTime }) => {
		try {
			const parsedDate = parseRelativeDate(newDate);
			const dayStart = `${parsedDate}T00:00:00+05:30`;
			const dayEnd = `${parsedDate}T23:59:59+05:30`;
			...
			const events = (await makeCalendarApiRequest(url)).items || [];
			const eventToReschedule = events.find(event =>
				(event.attendees || []).some((a: any) => a.email === email)
			);

			if (!eventToReschedule) {
				return { content: [{ type: "text", text: `❌ Could not find any event with attendee ${email} on ${parsedDate}.` }] };
			}

			const startDateTime = `${parsedDate}T${newStartTime}:00+05:30`;
			const endDateTime = `${parsedDate}T${newEndTime}:00+05:30`;

			await makeCalendarApiRequest(
				`https://www.googleapis.com/calendar/v3/calendars/primary/events/${eventToReschedule.id}`,
				{
					method: "PATCH",
					body: JSON.stringify({
						start: { dateTime: startDateTime, timeZone: "Asia/Kolkata" },
			...
			return { content: [{ type: "text", text: `✅ Appointment rescheduled to ${parsedDate} from ${newStartTime} to ${newEndTime}.` }] };
		} catch (error) {
			return { content: [{ type: "text", text: `❌ Failed to reschedule appointment. ${error instanceof Error ? error.message : 'Unknown error.'}` }] };
		}
	}
);
}
